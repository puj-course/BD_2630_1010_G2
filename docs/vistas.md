...
El presente documento justifica el propósito de cada una de las vistas implementadas en sql/vistas/vistas.sql, construidas sobre el Modelo Genérico Inicial de cinco entidades y sobre el esquema de referencia compartido del curso MORENOLUIS.FIFA_* (de solo lectura, véase README_GUIA_SERVIDOR.md).

Nota sobre la ocupación de estadios: las tablas MORENOLUIS.FIFA_ESTADIO y MORENOLUIS.FIFA_PARTIDO no incorporan ningún atributo de asistencia o aforo, y al ser de solo lectura no es posible agregarles una columna. Para la Vista 3 (VW_OCUPACION_ESTADIO), el equipo incorpora, en su propio esquema, una tabla auxiliar:
...
poblada con datos ficticios propios para cada id_partido de MORENOLUIS.FIFA_PARTIDO que se desee emplear en esta vista y en la Consulta 2 / Consulta 8. Constituye el único elemento agregado sobre el modelo inicial, documentado también en docs/documento_tecnico.md (Sección 2).
...
Propósito: calcular, para cada selección dentro de cada edición, la tabla de posiciones parcial completa, sin omitir ninguna de las columnas propias de una tabla de posiciones convencional, con los puntos calculados conforme al criterio 3-1-0 (victoria: 3 puntos; empate: 1 punto; derrota: 0 puntos).
...
Propósito: exponer el total de goles marcados por cada selección en cada edición.
...
Propósito: calcular el porcentaje de ocupación estimado de cada estadio (asistencia_real / capacidad × 100, promediado sobre los partidos disputados en él), a partir de la tabla propia ASISTENCIA_PARTIDO descrita anteriormente.
...
Propósito: presentar cada partido junto con su marcador (selección local, selección visitante y goles de cada una) y su sede (estadio y ciudad), en una única fila por partido.
...
Propósito: calcular, para cada selección de cada edición, los goles a favor, los goles en contra y la diferencia de gol acumulada a lo largo de sus partidos.
